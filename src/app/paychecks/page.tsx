...
export default function PaychecksPage () {
  ...
  function handleFormChange ( field: keyof PaycheckFormData, value: string | boolean ) {
    setForm( ( prev ) => {
      ...
      // When employee 401K % changes, auto-calculate the dollar amount
  ...
  async function handleSave () {
    ...
    // ── Sync 401K investment (employee contribution + employer match) ──────────
    const total401k = employee401k + employerMatch;
    if ( total401k > 0 ) {
      try {
        // Find the user's 401K investment record (type contains "401K")
    ...
    <div className="p-4 md:p-6 space-y-5 md:space-y-6 max-w-6xl mx-auto">
      ...
      {yearPaychecks.length === 0 ? (
        <EmptyState
          ...
          action={{ label: "Add Paycheck", onClick: () => setShowModal( true ) }}
        />
      ) : (
