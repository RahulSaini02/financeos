...
export default function InvestmentsPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      const [{ data: inv, error: invErr }, { data: goalsData, error: goalsErr }, { data: accts }] =
        await Promise.all([
          supabase
            .from("investments")
  ...
  async function handleSave(form: InvestmentForm) {
    ...
    // Sync linked account balance = sum of all investments now in that account
    const accountsToSync = new Set<string>();
    if (form.account_id) accountsToSync.add(form.account_id);
    // If account changed, also resync old account
  ...
  async function handleDelete(id: string) {
    ...
    // Resync account balance after deletion
